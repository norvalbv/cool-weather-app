...
const Rain = (): ReactElement => {
  const { id: aId, url: aUrl, hash: aHash } = useUniqueId();
  const { id: bId, url: bUrl, hash: bHash } = useUniqueId();
  const { id: cId, url: cUrl, hash: cHash } = useUniqueId();
  const { id: dId, url: dUrl, hash: dHash } = useUniqueId();
  const { id: eId, hash: eHash } = useUniqueId();
  const { id: fId, url: fUrl, hash: fHash } = useUniqueId();
  ...
  return (
    ...
    >
      <defs>
        ...
        <symbol id={fId} viewBox="0 0 129 57" overflow="visible">
          <path
      ...
      <use width="350" height="222" transform="translate(81 145)" xlinkHref={eHash} />
      <use width="129" height="57" transform="translate(191.5 343.5)" xlinkHref={fHash} />
    </svg>
  );
};
